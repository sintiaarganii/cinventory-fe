...
// ==================== DETAIL MODAL ====================
const MovementDetailModal = React.memo(({ show, onClose, movement }) => {
...
const StockMovements = () => {
  ...
  const handleViewDetails = useCallback((movement) => {
  ...
  // Stats data
  const stats = useMemo(() => {
    const totalIn = movements.filter((m) => m.transaction_type === "IN").length;
    const totalOut = movements.filter(
      (m) => m.transaction_type === "OUT",
    ).length;
    const totalTransfer = movements.filter(
      (m) => m.transaction_type === "TRANSFER",
    ).length;
    ...
    return [
      ...
      },
      {
        title: "Stock In",
        value: totalIn,
        subtitle: `${totalQuantityIn.toLocaleString()} units`,
        icon: TrendingDown,
      ...
      },
      {
        title: "Stock Out",
        value: totalOut,
        subtitle: `${totalQuantityOut.toLocaleString()} units`,
        icon: TrendingUp,
        color: "text-red-600",
        bg: "bg-red-50",
      },
...
export default React.memo(StockMovements);
