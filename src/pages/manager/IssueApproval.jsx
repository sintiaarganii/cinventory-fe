...
const IssueApproval = () => {
  ...
  const handleViewDetails = async (issue) => {
    try {
      ...
      setSelectedIssueDetails(result.details);
      setShowDetailsModal(true);
    } catch (error) {
      console.error("Error in handleViewDetails:", error);
      Swal.fire("Error!", "Failed to load issue details", "error");
    }
  };
  ...
  const handleApprove = async (id, code) => {
    const result = await Swal.fire({
      title: "Approve Issue?",
      text: `Are you sure you want to approve goods issue "${code}"?`,
      icon: "question",
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem("token");
        await api.put(
          `/goods-issues/approve/${id}`,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        Swal.fire("Approved!", "Goods issue has been approved.", "success");
        getGoodsIssues();
      } catch (error) {
        Swal.fire(
          "Error!",
  ...
  const handleReject = async (id, code) => {
    const result = await Swal.fire({
      title: "Reject Issue?",
      text: `Are you sure you want to reject goods issue "${code}"?`,
      icon: "warning",
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem("token");
        await api.put(
          `/goods-issues/reject/${id}`,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        Swal.fire("Rejected!", "Goods issue has been rejected.", "success");
        getGoodsIssues();
      } catch (error) {
        Swal.fire(
          "Error!",
          error.response?.data?.message || "Failed to reject issue",
          "error",
        );
      }
    }
  ...
  const stats = [
    ...
    {
      title: "Rejected",
      value: issues.filter((i) => i.status === "Rejected").length,
      icon: XCircle,
      color: "text-red-600",
      bg: "bg-red-50",
    },
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-slate-500 mt-2">
            Review and approve outgoing goods issues
          </p>
        </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          {item.status === "Pending" && (
                            <>
                              <button
                                onClick={() =>
                                  handleApprove(item.id, item.issue_code)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleReject(item.id, item.issue_code)
                                }
