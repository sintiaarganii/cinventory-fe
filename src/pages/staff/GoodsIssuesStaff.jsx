...
import GoodsIssuesStaffModal from "./GoodsIssuesStaffModal";
...
// ==================== ISSUE ROW ====================
const IssueRow = React.memo(({ item, index, onViewDetails, animateItems }) => {
...
const GoodsIssuesStaff = () => {
  ...
  const fetchDropdownData = async () => {
    try {
      ...
      const issuesRes = await api.get("/goods-issues", {
        params: { limit: 1000 },
        headers: { Authorization: `Bearer ${token}` },
      });
      if (issuesRes.data.success) {
        const codes = issuesRes.data.data.map((i) => i.issue_code);
      ...
      <main className="flex-1 ml-72 p-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-slate-500 mt-2">
            View and manage outgoing goods issues
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden transition-all duration-300">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-lg">No goods issues found</p>
              <p className="text-sm mt-1">Click "Create Issue" to add one</p>
            </div>
          ) : (
...
export default React.memo(GoodsIssuesStaff);
